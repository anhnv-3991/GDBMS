...
public:
	CUDAH GTuple();
	CUDAH GTuple(int64_t *tuple, GColumnInfo *schema_buff, int max_columns, int stride, char *secondary_storage = NULL);
...
CUDAH GTuple::GTuple(int64_t *tuple, GColumnInfo *schema_buff, int max_columns, int stride, char *secondary_storage = NULL)
{
	tuple_ = tuple;
...
CUDAH bool GTuple::setGNValue(GNValue value, int column_idx)
{
	...
	tuple_[column_idx * stride_] = value.m_data_;
...
{
	if (column_idx < columns_)
		return GNValue(schema_[column_idx].data_type, tuple_[column_idx * stride_], secondary_storage_);
