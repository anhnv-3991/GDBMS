...
GBlock::GBlock()
{
	...
	next = prev = NULL;
	stride_ = max_row_num_;
	schema_ = NULL;
}

__global__ void copyTable(int64_t *source, int64_t *destination, int row_num, int col_num, int source_stride, int destination_stride, int sbase_id, int dbase_id)
{
	int row_id = threadIdx.x + blockIdx.x * blockDim.x;
	int col_id = blockIdx.y;

	if (row_id < row_num && col_id < col_num)
		destination[col_id * destination_stride + row_id + dbase_id] = source[col_id * source_stride + row_id + sbase_id];
}
...
bool GBlock::addColumn()
{
	if (gpu_resident_) {
		if (col_num_ == max_col_num_) {
			...
			new_block->stride_ = new_max_row_num;
			...
			copyTable<<<grid, block>>>(primary_storage_, new_block->primary_storage_, copy_size, col_num_, stride_, new_block->stride_, 0, 0);
			...
			if (copy_size > 0) {
				...
				new_block2->stride_ = new_max_row_num;
				...
				copyTable<<<grid, block>>>(primary_storage_, new_block2->primary_storage_, copy_size, col_num_, stride_, new_block2->stride_, new_max_row_num, 0);
				checkCudaErrors(cudaGetLastError());
				checkCudaErrors(cudaDeviceSynchronize());
